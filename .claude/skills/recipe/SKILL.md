...
- **Timer tokens** — any concrete timer-able time reference in a step should use `{label|duration}` inline. Label is 1–3 words. Examples: `{Bake|25-35 minutes}`, `{Rest|30 minutes}`, `{Marinate|3 hours}`. Do NOT wrap intervals ("every 5 minutes"), vague times ("a few minutes"), or durations that don't benefit from a countdown timer (e.g. multi-day cold proofs like "1-3 days").
